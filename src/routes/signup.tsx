...
function SignUpPage() {
      ...
      fetch(`/api/check-username?username=${encodeURIComponent(value)}`)
        ...
        .then((d) => {
          if (d.available) {
            ...
            setUsernameError(d.error || 'Not available')
          }
        })
        .catch(() => {
          setUsernameStatus('idle')
  ...
  const canSubmit = usernameStatus === 'available' && displayName.trim().length > 0 && email && password && status !== 'loading' && status !== 'done'
    ...
    <div className="min-h-screen bg-black flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                ...
                {usernameStatus === 'taken' && <span className="text-red-400 text-sm">&#10005;</span>}
              </div>
            </div>
            {usernameError && <p className="text-xs text-red-400 mt-1">{usernameError}</p>}
            {usernameStatus === 'available' && <p className="text-xs text-emerald-400 mt-1">@{username} is available</p>}
            <p className="text-[11px] text-zinc-600 mt-1">3–20 characters. Letters, numbers, underscores, periods.</p>
          </div>
          ...
          >
            {status === 'loading' ? 'Creating account…' : status === 'done' ? 'Check your email' : 'Create Account'}
          </button>
        ...
        <p className="text-center text-[11px] text-zinc-600 mt-4 px-4 leading-relaxed">
          ...
          <Link to="/terms" className="underline underline-offset-2 hover:text-zinc-400">Terms</Link>,{' '}
          <Link to="/privacy" className="underline underline-offset-2 hover:text-zinc-400">Privacy Policy</Link>, and{' '}
          <Link to="/community-guidelines" className="underline underline-offset-2 hover:text-zinc-400">Guidelines</Link>.
        </p>
      </div>
    </div>
  )
