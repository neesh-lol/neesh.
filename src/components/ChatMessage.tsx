...
function Avatar({ name, url }: { name: string; url?: string }) {
  if (url) return <img src={url} alt={name} className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
  return (
    <div className="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center text-xs font-medium text-white flex-shrink-0">
...
export function ChatMessage({
  ...
  if (isMuted) {
    return (
      <div className={`flex items-start gap-3 ${grouped ? 'mt-0.5' : 'mt-4'} msg-enter group`}>
        {grouped ? <div className="w-8 flex-shrink-0" /> : <div className="w-8 h-8 rounded-full bg-zinc-800 flex-shrink-0" />}
  ...
  for (const r of reactions) {
    if (!groupedReactions[r.emoji]) groupedReactions[r.emoji] = { count: 0, users: [], hasOwn: false }
    groupedReactions[r.emoji].count++
    groupedReactions[r.emoji].users.push(r.displayName)
    if (r.userId === currentUserId) groupedReactions[r.emoji].hasOwn = true
  }
    ...
    >
      {grouped ? (
        ...
        <button className="flex-shrink-0" onClick={(e) => onAvatarClick(e, msg)}>
          <Avatar name={msg.displayName} url={msg.avatarUrl || undefined} />
        </button>
      )}
      <div className="min-w-0 flex-1">
        ...
        {!grouped && (
          <div className="flex items-center gap-2 mb-0.5">
            ...
            {isOwn && isDelivered && (
              <span className="text-[10px] text-zinc-600" title="Delivered">✓✓</span>
            )}
          </div>
        )}
            ...
            {Object.entries(groupedReactions).map(([emoji, data]) => (
              <button
                ...
                className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs transition-all reaction-btn ${
                  data.hasOwn ? 'bg-purple-500/20 border border-purple-500/40 text-purple-300' : 'bg-zinc-800 border border-zinc-700 text-zinc-400 hover:border-zinc-600'
                }`}
      ...
      {showActions && (
        <div ref={actionsRef} className="absolute right-0 -top-3 flex items-center gap-0.5 bg-zinc-800 border border-zinc-700 rounded-lg px-1 py-0.5 shadow-lg z-10 action-bar">
          <button
          ...
          {showContextMenu && !isOwn && (
            <div className="absolute top-8 right-0 bg-zinc-800 border border-zinc-700 rounded-lg py-1 shadow-xl z-20 min-w-[140px] context-menu">
              <button
                onClick={() => { onReport(msg); setShowContextMenu(false) }}
                className="w-full px-3 py-1.5 text-left text-xs text-zinc-300 hover:bg-zinc-700 flex items-center gap-2 transition-colors"
              >
                <Flag size={12} /> Report
              </button>
              <button
                onClick={() => { onMute(msg.userId); setShowContextMenu(false) }}
                className="w-full px-3 py-1.5 text-left text-xs text-zinc-300 hover:bg-zinc-700 flex items-center gap-2 transition-colors"
              >
                <VolumeX size={12} /> Mute
              </button>
              <button
                onClick={() => { onBlock(msg.userId); setShowContextMenu(false) }}
              ...
              {isAdmin && onDelete && (
                <button
                  onClick={() => { onDelete(msg); setShowContextMenu(false) }}
                  className="w-full px-3 py-1.5 text-left text-xs text-red-400 hover:bg-zinc-700 flex items-center gap-2 transition-colors"
                >
  ...
  return (
    <div className="flex items-center gap-2 px-5 py-1.5 text-xs text-zinc-500 typing-indicator">
      <span className="flex gap-0.5">
        ...
        <span className="typing-dot w-1.5 h-1.5 rounded-full bg-zinc-500" style={{ animationDelay: '0.15s' }} />
        <span className="typing-dot w-1.5 h-1.5 rounded-full bg-zinc-500" style={{ animationDelay: '0.3s' }} />
      </span>
      <span>{text}</span>
    </div>
  )
...
export function ReplyPreview({ msg, onCancel }: { msg: ChatMessageData; onCancel: () => void }) {
  return (
    <div className="flex items-center gap-2 px-5 py-2 bg-zinc-900 border-t border-zinc-800 reply-preview">
      ...
      <button onClick={onCancel} className="text-zinc-500 hover:text-white transition-colors text-lg leading-none">&times;</button>
    </div>
  )
}
